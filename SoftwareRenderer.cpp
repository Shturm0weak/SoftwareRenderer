...
namespace objl
{
	...
	public:
		...
		static sr::Mesh* Load(const std::string& filePath)
		{
			objl::Loader loader;
			...
			if (loadout)
			{
				for (size_t i = 0; i < loader.LoadedMeshes.size(); i++)
				{
					...
					{
						mesh->m_Vertices.push_back({ curMesh.Vertices[j].Position.X, curMesh.Vertices[j].Position.Z, curMesh.Vertices[j].Position.Y });
						mesh->m_Normals.push_back({ curMesh.Vertices[j].Normal.X, curMesh.Vertices[j].Normal.Z, curMesh.Vertices[j].Normal.Y });
					}
